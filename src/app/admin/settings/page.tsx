      ...
      <div className="p-8 md:p-12 max-w-5xl mx-auto space-y-16">
        ...
        <div className="glass-card p-10 border-[var(--admin-border)]">
          ...
          <div className="space-y-8">
            ...
            <InputField label="Location" value={settings.location} onChange={(v:any) => setSettings({...settings, location: v})} />
            
            <div className="space-y-4 pt-6 border-t border-[var(--admin-border)]">
              <label className="text-[10px] font-black text-[var(--admin-muted)] uppercase tracking-widest ml-1">Technical Skills</label>
              <div className="flex flex-wrap gap-2 mb-4">
                {(Array.isArray(settings.skills) ? settings.skills : []).map((skill: string, i: number) => (
                  <span key={i} className="px-3 py-1 bg-purple-600/10 text-purple-400 rounded-lg text-xs font-bold border border-purple-500/20 flex items-center gap-2">
                    ...
                    }} className="hover:text-red-400 transition-colors"><Trash2 size={10} /></button>
                  </span>
                ))}
              </div>
              <div className="flex gap-2">
                <input 
                  type="text" 
                  placeholder="Add skill (e.g. Docker) + Enter" 
                  onKeyDown={(e: any) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                  ...
                  className="bg-[var(--admin-card)] border border-[var(--admin-border)] rounded-xl px-4 py-2 text-xs text-[var(--admin-text)] outline-none focus:border-purple-500/50 w-full max-w-xs" 
                />
              </div>
            </div>
          </div>
        ...
        {/* Dynamic Statistics */}
        <div className="glass-card p-10 border-[var(--admin-border)]">
          <SectionTitle icon={TrendingUp} title="Key Metrics" subtitle="Stats displayed on your profile" />
          <div className="space-y-6">
             <div className="grid md:grid-cols-2 gap-6">
                {(Array.isArray(settings.stats) ? settings.stats : []).map((stat: any, i: number) => (
                  <div key={i} className="p-6 bg-[var(--admin-card)] border border-[var(--admin-border)] rounded-2xl relative group">
                    <button 
                      onClick={() => {
                        const newStats = settings.stats.filter((_: any, index: number) => index !== i);
                        setSettings({...settings, stats: newStats});
                      }}
                      className="absolute top-4 right-4 text-[var(--admin-muted)] hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
                    >
                      <Trash2 size={14} />
                    </button>
                    <div className="grid gap-4">
                      <div className="space-y-1">
                        <label className="text-[9px] font-black text-[var(--admin-muted)] uppercase tracking-widest">Label</label>
                        <input 
                          value={stat.label} 
                          onChange={(e) => {
                          ...
                          className="w-full bg-transparent border-none outline-none text-sm font-bold text-[var(--admin-text)]"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] font-black text-[var(--admin-muted)] uppercase tracking-widest">Value</label>
                        <input 
                          value={stat.value} 
                          onChange={(e) => {
                            const newStats = [...settings.stats];
                          ...
                          className="w-full bg-transparent border-none outline-none text-2xl font-black text-purple-500"
                        />
                      </div>
                    </div>
                  </div>
             ...
             <button 
              onClick={() => setSettings({...settings, stats: [...(settings.stats || []), { label: "New Stat", value: "0" }]})}
              className="w-full py-4 border-2 border-dashed border-[var(--admin-border)] rounded-2xl text-[var(--admin-muted)] hover:text-purple-500 hover:border-purple-500/30 transition-all font-bold text-xs"
             >
               + Add Metric
             </button>
          </div>
