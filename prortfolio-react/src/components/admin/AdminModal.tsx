...
export function AdminModal({ 
  isOpen, onClose, title, subtitle, children, maxWidth = "max-w-3xl" 
}: AdminModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 overflow-y-auto">
          ...
          >
            <div className="flex items-center justify-between mb-8 md:mb-10">
              <div>
                <h2 className="text-2xl md:text-3xl font-black text-[var(--admin-text)]">{title}</h2>
                <p className="text-[10px] md:text-xs text-[var(--admin-muted)] mt-1 uppercase tracking-widest">{subtitle}</p>
              </div>
              <button 
                onClick={onClose} 
